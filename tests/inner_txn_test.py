import pytest
from pyteal import *
from pytealext.inner_transactions import *
...
def test_MakeInnerAssetTransferTxn():
    ...
    for param in params:
        ast_actual = MakeInnerAssetTransferTxn(
            **{param: App.localGet(Int(0), Bytes("test"))}
        )
...
def test_MakeInnerAssetFreezeTxn():
    ...
    for param in params:
        ast_actual = MakeInnerAssetFreezeTxn(
            **{param: App.localGet(Int(0), Bytes("test"))}
        )
...
def test_MakeInnerAssetConfigTxn():
    ...
    for param in params:
        ast_actual = MakeInnerAssetConfigTxn(
            **{param: App.localGet(Int(0), Bytes("test"))}
        )
